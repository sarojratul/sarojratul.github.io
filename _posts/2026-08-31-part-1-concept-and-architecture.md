...
excerpt: "One double-headed arrow instead of two, a sweep borrowed from a 1965 Thunderbird, the system architecture, the power budget, and how every part was checked against its datasheet before ordering."
permalink: /turn-signal/part-1-design/
redirect_from:
  - /2026/08/28/concept-specifications-architecture.html
---

This entry is the design: what the thing is, how it is split up, what it runs on, and why each part was chosen. It was all settled on paper before any hardware existed.

> **Reading note.** Some of these numbers changed once real hardware was on the bench. Following the log's own rule (superseded, not deleted), the original figures stay and each change is marked **Update** with a link to the entry where it happened.
...
I killed it on 31 August, after the first simulations had already run (the effect that had on the work is in [Part 2](/turn-signal/part-2-simulation/#redesign-day-mid-stream)). Two reasons, one aesthetic and one electrical:
...
> **Update, 20 September:** these read far too fast on the real panel. After tuning by eye and checking on a scope, the final values are `STEP_MS = 450`, `HOLD_MS = 788`, `CYCLE_MS = 1688`. See [Part 6](/turn-signal/part-6-esp32-drives-the-panel/#tuning-the-sweep-by-eye-then-checking-it-with-a-scope).
...
| **Controller** | ESP32-C3 | ESP32-C3, GPIO 3 / 4 / 5 / 6 (**Update:** originally 2 / 4 / 5 / 6, see below) |
...
*The whole system. Note the asymmetry: the transmitter is a switch, a radio and a regulator, while everything that decides anything lives on the receiver. The 322.9 µA and the 2.38 mV are the LTspice predictions that Parts 3 and 4 go on to check against the bench.*
{: .caption}
...
> **Update, 20 September:** GPIO 2 turned out to be an ESP32-C3 strapping pin that Espressif recommends pulling *up*, and segment A's gate network puts a 10 kΩ pulldown on it. Segment A moved to GPIO 3 before any wire touched the board. See [Part 6](/turn-signal/part-6-esp32-drives-the-panel/#a-gpio-pin-the-datasheet-caught-before-i-did). The block diagram above shows the corrected pin set.
...
> **Update, 20 September:** on the bench the real LEDs dropped about 2.08 V each rather than the modelled 1.99 V, so every segment draws less than this table says. Measured at 4.8 V: segment A or D about 80 to 89 mA, segment B or C 53 mA, all four together 265 mA. The table stays as the ceiling from the idealised model. See [Part 5](/turn-signal/part-5-full-panel/).
...
## Choosing parts: how I used AI, and why that is a skill rather than a shortcut
...
- **The SOT-23 problem nobody mentioned.** The AO3400A is a surface-mount part. It does not plug into a breadboard. That fact appears in no BOM; you find it when you look at the package drawing in the datasheet. I ordered **SOT-23-to-DIP adapters** and header pins to go with them. (What that cost me is in [Part 3](/turn-signal/part-3-soldering-and-panel-build/).)
...
## The build guide and the work plan
