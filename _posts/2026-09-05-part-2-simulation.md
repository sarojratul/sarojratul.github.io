...
excerpt: "LTspice for the analog half, Wokwi for the state machine, Tinkercad for the breadboard layout, and a mid-stream redesign that did not waste the work already done."
permalink: /turn-signal/part-2-simulation/
redirect_from:
  - /2026/08/29/simulation-and-virtual-prototyping.html
---

Before spending anything on a breadboard, every part of the design went through a simulator: LTspice for the electronics, Wokwi for the firmware, and Tinkercad for the physical breadboard layout. Each one proves something different, and each one has something it cannot tell you.
...
## LTspice, the analog half (Stage A2)
...
*Step 1 as it actually ran. Two AMBER diodes in series with a 33 Ω resistor on a 4.8 V source, and the operating point window reporting I(D1) = I(D2) = I(R1) = 0.0247755 A. That is the 24.78 mA the whole design is built on.*
{: .caption}

**Step 1, model the LED.** The Lumex part has no SPICE model, so I built one from the datasheet's V<sub>f</sub> curve:
...
**Step 2, DC sweep, 4.0 V to 5.0 V.** This answers "what happens as the batteries die?"
...
*The sweep itself, `.dc V1 4.0 5.0 0.05`, plotting the voltage across the 33 Ω resistor. Divide by 33 to get string current.*
{: .caption}
...
**Step 3, a full segment with the real MOSFET.** Three strings, an AO3400A low-side switch, 3.3 V on the gate. The AO3400A ships as a `.SUBCKT`, which took some fighting (see the gotchas below).
...
*Segment B/C in full: three strings, three 33 Ω resistors, the AO3400A as a low-side switch, and the 220 Ω gate resistor with the 10 kΩ pulldown. Three numbers in that operating point window matter. I(V1) = 0.0741613 A is the segment current. V(n002) = 0.00238018 V is V<sub>DS</sub>, the 2.38 mV. And I(R4) = I(R5) = 0.000322896 A is the gate current, which is the 322.9 µA I would go on to measure on the real breadboard nine days later ([Part 3](/turn-signal/part-3-soldering-and-panel-build/#test-1-the-gate-networks-pass)).*
{: .caption}
...
## Redesign day, mid-stream

<p class="entry-date">31 August 2026</p>

This is the day the two-arrow design died and the shared double-headed arrow replaced it (the reasoning is in [Part 1](/turn-signal/part-1-design/#two-arrows-became-one)). It happened *after* I had already simulated and documented the old one.
...
## Wokwi, the firmware half (Stage A3)
...
I did not know then how directly I would need that a week later ([Part 4](/turn-signal/part-4-floating-ground/)).
...
> **Update, 20 September:** partly settled. The timing was tuned by eye on the real panel and ended at a 788 ms full-arrow hold in a 1688 ms cycle. See [Part 6](/turn-signal/part-6-esp32-drives-the-panel/#tuning-the-sweep-by-eye-then-checking-it-with-a-scope). The outdoor night check is still to come.
...
> **Update, 20 September:** the lock did not survive the datasheet. GPIO 2 is a strapping pin, so the final set is **GPIO 3/4/5/6**. See [Part 6](/turn-signal/part-6-esp32-drives-the-panel/#a-gpio-pin-the-datasheet-caught-before-i-did).

---

## Tinkercad, which nobody suggested
...
*The full rehearsal in Tinkercad Circuits: 16 strings of two LEDs, 16 resistors, four MOSFETs, the 470 µF bulk capacitor, a 4.8 V supply for the panel rail and a second supply for gate injection. This is what let me separate a wrong circuit from a leg in the wrong hole before touching real parts.*
{: .caption}
...
> **What Tinkercad proves and does not prove:** it proves the topology and the wiring plan. It proves nothing about real components, real currents, thermals, or whether a stranded wire is making contact. I knew that when I wrote it down. Parts 3 and 4 are about finding out how much it does not prove.
